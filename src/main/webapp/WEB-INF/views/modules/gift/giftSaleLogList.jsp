...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/gift/giftSaleLog/">礼包购买记录列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="giftSaleLog" action="${ctx}/gift/giftSaleLog/" method="post" class="breadcrumb form-search">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead>
			<tr>
				...
				<th>总金额</th>
				<th>创建时间</th>
				<shiro:hasPermission name="gift:giftSaleLog:edit"><th>操作</th></shiro:hasPermission>
			</tr>
		</thead>
		<tbody>
		<c:forEach items="${page.list}" var="giftSaleLog">
			<tr>
				...
				<td>
					<fmt:formatDate value="${giftSaleLog.createDate}" pattern="yyyy-MM-dd HH:mm:ss"/>
				</td>
				<shiro:hasPermission name="gift:giftSaleLog:edit"><td>
					<c:if test="${giftSaleLog.flag == '0'}">
						<a href="${ctx}/gift/giftSaleLog/checkPass?id=${giftSaleLog.id}">审核通过</a>
						<a href="${ctx}/gift/giftSaleLog/orderCancel?id=${giftSaleLog.id}" onclick="return confirmx('确认要作废该订单吗？', this.href)">作废</a>
					</c:if>
